...
var LightController = {
  ...
  pincode: "111-22-333",
  username: "B8:27:EB:8D:1A:F9", // MAC like address used by HomeKit to differentiate accessories. 
...
wpi.pinMode(ledPin2, wpi.OUTPUT);
 if (wpi.digitalRead(ledPin2) == 0) {
 wpi.digitalWrite(ledPin2, wpi.HIGH);
 } else {
 wpi.digitalWrite(ledPin2, wpi.LOW);
 };
wpi.pinMode(ledPin3, wpi.OUTPUT);
 if (wpi.digitalRead(ledPin3) == 0) {
 wpi.digitalWrite(ledPin3, wpi.HIGH);
 } else {
 wpi.digitalWrite(ledPin3, wpi.LOW);
 };
  callback();
  })
